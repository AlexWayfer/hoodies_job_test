...
RSpec.describe Solution::Session do
  ...
  describe '#initialize' do
    ...
    context 'with args' do
      let(:args) { [user_id: user_id, session_id: session_id, time: time, date: date] }
            ...
            context 'when `time` is equal to 0' do
              ...
              context 'with `date` as `Date`' do
                ...
                it do
                  expect { session }.to raise_error(
                    Dry::Struct::Error,
                    /0 \(Integer\) has invalid type for :time violates constraints/
                  )
                end
              end
            end
